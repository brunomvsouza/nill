...
window.nill = (function (window, document) {
  ...
  /**
   * Polyfill for document.querySelectorAll('img[' + imageSrcAttribute + ']')
   * Fucking IE7 =/
   */
  var getImagesByAttributeName = function(attributeName) {
    var matchingImages = [],
      $images = document.getElementsByTagName('img'),
      length = $images.length;

    for (var i = 0; i < length; i++) {
      if ($images[i].getAttribute(attributeName)) {
        matchingImages.push($images[i]);
      }
    }
  ...
  var $imgs = (document.querySelectorAll) ? 
    document.querySelectorAll('img[' + imageSrcAttribute + ']') : 
    getImagesByAttributeName(imageSrcAttribute);
